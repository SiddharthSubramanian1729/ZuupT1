...
async def receive_state(data: dict):
    ...
    if "is_sunlit" in data:
        ...
        satellite_state["latitude"] = data.get(
            "latitude", satellite_state["latitude"])
        satellite_state["longitude"] = data.get(
            "longitude", satellite_state["longitude"])
        satellite_state["altitude_km"] = data.get(
            "altitude_km", satellite_state["altitude_km"])
        satellite_state["debris_warning"] = data.get(
            "hazard_debris_warning", False)

    # --- Power data ---
    if "flywheel_charge_pct" in data:
        # their battery = flywheel
        satellite_state["battery"] = data["flywheel_charge_pct"]
        solar_average_kw = (
            data.get("solar_port_wing_kw", 0) +
            data.get("solar_starboard_wing_kw", 0)
        ) / 2
        ...
        satellite_state["blackout"] = data.get(
            "alerts", {}).get("blackout_critical", False)

    # --- Thermal data ---
    if "avg_cluster_temp_c" in data:
        # main temp we use
        satellite_state["temperature"] = data["avg_cluster_temp_c"]
        satellite_state["max_node_temp"] = data.get(
            "max_node_temp_c", satellite_state["max_node_temp"])
        satellite_state["node_temperatures"] = data.get(
            "node_temperatures_c", [])
        ...
        if mission_status == "CRITICAL_NODE_FAILURE" and not critical_node_failure_active:
            satellite_state["healthy_gpus"] = max(
                0, satellite_state.get("healthy_gpus", 8) - 2)
...
def calculate_health() -> int:
    ...
    score = (gpus / 8) * 60 + (min(battery, 100) / 100) * \
        30 + (max(0, 85 - temp) / 85) * 10
